...
A nuvem que será utilizada neste trabalho é a \Sigla{Google Cloud Platform}{GCP} porque ela entrega serviços que atendem perfeitamente os requisitos do trabalho, por exemplo, APIs de busca de locais que utilizam a base de dados do \textit{Google}.

Neste projeto será utilizado um \Sigla{\textit{Backend-as-a-Service}}{BaaS}, que é um serviço de \textit{back end} gerenciado por uma empresa. Como a provedora de nuvem escolhida nesse projeto é a GCP, o serviço de BaaS dela se chama \textit{Firebase}.
...
Outro serviço que será utilizado é o \Sigla{\textit{Firebase Cloud Messaging}}{FCM}, que é responsável pelo envio de notificações aos usuários que forem possivelmente expostos a alguma doença infecciosa.
...
O modelo de dados do \textcite{FirestoreDataModel} segue o formato \Sigla{JavaScript Object Notation}{JSON} e é estruturado em entidades denominadas documentos e coleções. A \Figura{fig:exemplofirestorejson} demonstra um exemplo dessa estrutura.
